...
Store подключён к UI: `HomePage` читает `todos` через `useAppSelector` и диспатчит `todoAdded` / `todoToggled` / `todoDeleted` вместо `setState`. Источник истины для todos теперь в Redux.

`filter` пока остаётся в локальном `useState` — намеренный контраст (todos через store, filter через props на одном экране). Следующие шаги: перенести `filter` в свой slice, выразить `visibleTodos` через селекторы и затем убрать prop drilling (компоненты обращаются к store напрямую).
